...
from shacs_bot.agent.planner import PlanStep
...
def test_policy_blocks_planned_workflow_when_daily_limit_exceeded(tmp_path: Path) -> None:
    stub = _StubProvider(_valid_plan_json())
    loop = _make_loop(
        stub,
        tmp_path,
        policy_config=PolicyConfig(enabled=True, daily_cost_limit=1.0),
    )
    plan = AgentLoop._classify_request("먼저 전체 데이터를 수집하고 그 다음에 정리해서 보내줘")

    enforced_plan, reason = loop._enforce_policy_on_plan(
        plan,
        channel="cli",
        sender_id="user-1",
        chat_id="direct",
        metadata={},
        daily_cost_override=1.0,
    )

    assert enforced_plan is None
    assert isinstance(reason, str) and "정책에 의해 워크플로우 생성이 제한되었습니다" in reason


def test_policy_blocks_workflow_step_when_daily_limit_exceeded(tmp_path: Path) -> None:
    stub = _StubProvider(_valid_plan_json())
    loop = _make_loop(
        stub,
        tmp_path,
    ...
    step = PlanStep(kind="research", description="정보 수집", depends_on=[])

    reason = loop._enforce_policy_on_workflow_step(
        step,
        channel="cli",
        chat_id="direct",
        actor_metadata={"senderId": "user-1", "userId": "user-1", "isDm": True},
        daily_cost_override=1.0,
    )

    assert isinstance(reason, str) and "정책에 의해 워크플로우 step 실행이 제한되었습니다" in reason


def test_policy_allows_planned_workflow_for_trusted_channel(tmp_path: Path) -> None:
    stub = _StubProvider(_valid_plan_json())
    loop = _make_loop(
        stub,
        tmp_path,
    ...
    enforced_plan, reason = loop._enforce_policy_on_plan(
        ...
        chat_id="room-1",
        metadata={},
        daily_cost_override=0.1,
    )

    assert enforced_plan is not None
    assert reason is None


def test_policy_allows_workflow_step_for_low_trust_low_risk(tmp_path: Path) -> None:
    stub = _StubProvider(_valid_plan_json())
    loop = _make_loop(
        stub,
        tmp_path,
    ...
    reason = loop._enforce_policy_on_workflow_step(
        ...
        chat_id="room-1",
        actor_metadata={"senderId": "user-9", "userId": "user-9", "isDm": False},
        daily_cost_override=0.1,
    )
...
async def test_wait_until_no_meta_normalized_from_description(tmp_path: Path) -> None:
    ...
    assert isinstance(iso, str) and iso, f"wait_until 정규화 실패: step_meta={wu.step_meta!r}"

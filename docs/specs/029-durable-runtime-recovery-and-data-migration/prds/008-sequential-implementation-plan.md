...
5. Spec 030은 unified redaction/typed secret ref를, Spec 035는 config/profile migration과 formal runtime layout/marker location을 소유한다. 029는 현재 shared redaction, config compatibility, runtime path-helper boundary를 소비해 독립적으로 닫고, 후속 spec은 같은 boundary를 확장한다.
...
6. Spec 030/035 미완료는 029의 current boundary 검증을 막지 않으며, 후속 owner 구현은 029를 다시 열지 않고 adapter 결과를 확장한다. 030은 redaction taxonomy를, 035는 physical path/layout ownership을 계속 소유한다.

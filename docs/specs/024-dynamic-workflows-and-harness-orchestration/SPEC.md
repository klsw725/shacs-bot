...
2026-07-15 기준 `shacs-workflow` crate에는 workflow state, harness plan, admission, checkpoint/resume, child graph, barrier, verifier gate, structured verifier evidence contract, worktree decision, budget/model route snapshot, runtime enforcement helper, recipe readiness, quarantine/permission ceiling, replay-safe diagnostics manifest, projection helper, and Spec 024 release evidence helper가 PRD009 runtime execution bucket까지 구현되어 있다. `crates/shacs-projection`에는 owner `024`와 redaction-valid evidence만 인정하는 projection-side Spec 024 release checklist가 있고, `crates/shacs-session` UX diagnostics는 persisted metadata에서 replay-safe diagnostics refs와 sanitized runtime workflow projection만 투영한다. `crates/shacs-core/src/runtime/workflow.rs`는 malformed plan을 fail-closed로 거부하는 read-only runtime path와 live child/verifier subagent를 실행하는 AgentLoop path를 제공한다.

현재 구현된 runtime은 pattern마다 별도 native engine을 두지 않고 dependency/barrier를 표현한 bounded DAG를 공통 scheduler로 실행한다. `tournament`는 이 경계에서 pre-expanded static DAG만 허용한다. Live AgentLoop는 metadata-provided workflow와 deterministic automatic read-only admission/planning 모두에서 child/verifier execution, runtime budget/timeout/model-route enforcement, skill recipe hint selection, pending workflow checkpoint, runtime workflow metadata를 기록한다. `PrivilegedActorSeparated` plan은 sanitizer step의 완료 output을 digest 검증한 뒤 그 sanitized handoff만 privileged child input으로 사용한다. Write-capable child는 symlink escape를 거부한 isolated git worktree에서만 실행되고 parent checkout을 자동 변경하지 않으며 diff evidence와 `PendingParentReview` merge handoff를 남긴다.

Runtime resume은 plan digest, completed child ids, worktree refs, recipe source refs를 재검증하고 evidence-valid completed step을 건너뛴다. Write-capable 또는 ambiguous checkpoint는 visible blocked state로 닫는다. Cancellation은 이미 완료된 child와 worktree/merge evidence를 보존한다. Runtime diagnostics는 harness/child/verifier digest, barrier, recipe, tool scope, synthesis, merge, cleanup decision을 persisted replay-safe manifest로 남긴다. CLI session inspect/diagnostics, local API session detail/diagnostics, channel outbound helper, and `shacs-tui` plain-text view는 같은 sanitized projection vocabulary를 소비한다. 명시적 비범위는 provider-native workflow beta, JavaScript workflow interpreter, native dynamic tournament bracket, hosted analytics/dashboard, organization-admin approval console, and child diff의 parent checkout 자동 merge다.
...
- Rust 검증은 관련 범위 기준 `cargo fmt --manifest-path crates/Cargo.toml --all -- --check`, `cargo clippy --manifest-path crates/Cargo.toml --workspace --all-targets -- -D warnings`, `cargo test --manifest-path crates/Cargo.toml --workspace`처럼 workspace manifest와 범위를 명시해 통과한다.

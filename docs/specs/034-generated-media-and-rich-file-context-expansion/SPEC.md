...
이 문서는 개인용 self-hosted runtime을 기준으로 한다. 사용자는 자신의 provider 설정, media root, diagnostics, local files를 직접 확인할 수 있어야 한다. CDN, public gallery, hosted editor 같은 원격 제품 표면은 기본 전제가 아니다.
...
6. 현재 generated media artifact는 raw bytes, raw base64, 만료성 remote URL을 session text로 직접 남기지 않는 방향을 따른다.
...
1. Generated media는 session text가 아니라 artifact record와 redacted reference로 남아야 한다.
2. Provider raw response, raw base64 image, signed URL 전체, raw token은 session message, tool result, diagnostics bundle에 그대로 저장하면 안 된다.
...
6. Remote URL provider output은 만료 URL을 장기 artifact처럼 표시하면 안 된다. Download를 선택하면 기존 network/SSRF guard, scheme allowlist, redirect별 재검증, byte/MIME cap을 통과하고 credential/header forwarding을 금지해야 하며 guard를 사용할 수 없으면 거절한다.
...
3. Source image, mask image, variation input은 media root containment와 provenance 검사를 통과해야 한다.
4. Partial streaming images는 started, partial, final, failed, cancelled 같은 상태를 구분해야 한다.
5. Remote URL output policy는 최소 세 선택지를 명시해야 한다. 기존 network/SSRF guard와 scheme/redirect/byte/MIME/credential-forwarding 제한을 통과한 download 후 persisted artifact 저장, redacted remote reference만 보존, provider output 거절.
6. Generated artifact metadata는 artifact id, kind, media root relative path, MIME, byte length, sha256, provider id, model id, source artifact ids, generation options summary, created at, redaction status를 포함해야 한다.
7. Diagnostics와 inspect는 artifact를 설명할 수 있어야 하지만 raw payload와 secret을 숨겨야 한다.
...
9. Video analyzer capability는 runtime에 주입 가능한 abstraction이어야 하며, analyzer missing을 정상 status로 설명해야 한다.
10. Replay는 live provider URL을 다시 따라가지 않고 recorded artifact metadata와 digest evidence를 사용해야 한다.
...
4. Remote URL output test가 private/link-local/loopback target 차단, redirect별 재검증, scheme allowlist, byte/MIME cap, credential/header 미전달, guard 부재 시 거절, persisted reference 정책을 검증한다.
5. Artifact persistence test가 metadata, digest, source chain, redaction status를 검증한다.
6. Diagnostics test가 raw base64, signed URL, token, provider raw response 전체가 빠졌는지 확인한다.

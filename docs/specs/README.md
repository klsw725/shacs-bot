...
- 024 구현 상태: `shacs-workflow` typed contract/helpers, durable checkpoint/resume/diagnostics, PRD009-inclusive release evidence checklist, `shacs-projection` projection-side release checklist, `shacs-session` persisted diagnostics ref projection, `shacs-core` read-only runtime workflow smoke, metadata-gated live AgentLoop child/verifier execution, skill recipe discovery, and CLI/local API recipe projection은 구현되었다. 일반 user turn에서 plan을 자동 생성/선택하는 full dynamic workflow product, write-capable git worktree merge handoff, structured verifier evidence, TUI/channel progress projection, and interruption/restart resume orchestration은 open 상태다.

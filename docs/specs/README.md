...
- 024 구현 상태: Rust self-hosted scope에서 closed 상태다. `shacs-workflow` typed contracts/release checklist, shared bounded DAG scheduling, verifier gate, skill recipe hint selection, sanitizer-to-privileged handoff, permission ceiling, isolated worktree/parent-review merge handoff, evidence-valid resume, cancellation evidence preservation, persisted replay-safe diagnostics/cleanup evidence, and CLI/local API/channel/TUI/session projection이 실제 runtime path에 연결되어 있다. `tournament`는 pre-expanded bounded static DAG만 지원한다. Provider-native workflow beta, JavaScript workflow interpreter, native dynamic tournament bracket, hosted dashboard, organization-admin approval console, and child diff의 parent checkout 자동 merge는 비범위다.

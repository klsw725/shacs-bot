...
현재 판정에서는 56 coverage row와 historical Spec031-owned command/artifact row가 매핑되어 있지만 Specs030/031/032/033/034 external read audit가 blocked라서 runner는 nonzero로 끝납니다. Current dirty state가 있으면 `triage/dirty-worktree.json`도 기록되지만 `BlockedExternalEvidence`를 가리지 않습니다. Spec029 audit만 현재 PASS입니다.

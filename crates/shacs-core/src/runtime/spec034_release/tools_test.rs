...
fn fake_path_cannot_replace_resolved_cargo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let status = std::process::Command::new(std::env::current_exe()?)
        .arg("--exact")
        .arg("runtime::spec034_release::tools::tests::fake_path_child")
        .arg("--nocapture")
        .env(CHILD, "fake-path")
        .env("PATH", root.path())
        .status()?;
...
fn git_environment_injection_is_not_inherited() -> Result<(), Box<dyn std::error::Error>> {
    let status = std::process::Command::new(std::env::current_exe()?)
        .arg("--exact")
        .arg("runtime::spec034_release::tools::tests::git_environment_child")
        .arg("--nocapture")
        .env(CHILD, "git-env")
...
fn ambient_cargo_home_wrapper_cannot_run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let status = std::process::Command::new(std::env::current_exe()?)
        .arg("--exact")
        .arg("runtime::spec034_release::tools::tests::ambient_cargo_home_wrapper_child")
        .arg("--nocapture")
...
fn tmpdir_cargo_config_is_ignored() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let status = std::process::Command::new(std::env::current_exe()?)
        .arg("--exact")
        .arg("runtime::spec034_release::tools::tests::tmpdir_cargo_config_child")
        .arg("--nocapture")

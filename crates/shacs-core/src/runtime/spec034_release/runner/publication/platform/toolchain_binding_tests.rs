...
fn assert_tool_replace_restore(name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let toolchain = crate::runtime::spec034_release::tools::ResolvedToolchain::resolve_tools_for_test(
        home,
        cargo_home,
        target,
        cargo.clone(),
    ...
    let approved = crate::runtime::spec034_release::artifacts::ArtifactSnapshot::capture(
        staging.path(),
    )?;

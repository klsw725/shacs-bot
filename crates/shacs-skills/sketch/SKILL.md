...
metadata.shacs.imported_from: creative/sketch
---
...
**Verify variants visually — use browser-capable tools.** Don't just write HTML and hope it renders; load each variant and look at it:

```
configured browser MCP tool(url="file:///absolute/path/to/sketches/001-calm-editorial/index.html")
configured browser MCP tool(question="Does this layout look clean and readable? Any visible bugs (overlapping text, unstyled elements, broken images)?")
```

`configured browser MCP tool` returns an AI description of what's actually on the page plus a screenshot path — catches layout bugs that pure source inspection misses (e.g. a font import that silently failed, a flex container that collapsed). Fix and re-navigate until each variant looks right.
...
```
exec("mkdir -p sketches/001-calm-editorial")
write_file("sketches/001-calm-editorial/index.html", "<!doctype html>...")
write_file("sketches/001-calm-editorial/README.md", "## Variant: Calm editorial\n...")
configured browser MCP tool(url="file://$(pwd)/sketches/001-calm-editorial/index.html")
configured browser MCP tool(question="How does this look? Any obvious layout issues?")
```

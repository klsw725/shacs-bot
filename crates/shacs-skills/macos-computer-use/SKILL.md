...
metadata.shacs.imported_from: apple/macos-computer-use
disabled: true
...
- Web automation you can do via configured browser MCP tools — those use a real
...
- File edits — use `read_file` / `write_file` / `edit_file`, not `type` into
  an editor window.

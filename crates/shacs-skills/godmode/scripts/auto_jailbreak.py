...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SHACS_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SHACS_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
SHACS_HOME = Path(os.getenv("SHACS_HOME", Path.home() / ".hermes"))
CONFIG_PATH = SHACS_HOME / "config.yaml"
PREFILL_PATH = SHACS_HOME / "prefill.json"

...
metadata.shacs.imported_from: productivity/google-workspace
---
...
```bash
GSETUP="python <workspace>/builtin_skills/google-workspace/scripts/setup.py"
```
...
`~/.shacs-bot/google_oauth_last_url.txt`.
...
- Token is stored at `~/.shacs-bot/google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/.shacs-bot/google_oauth_pending.json` until exchange completes.
- If `gws` is installed, `google_api.py` points it at the same `~/.shacs-bot/google_token.json` credentials file. Users do not need to run a separate `gws auth login` flow.
...
```bash
GAPI="python <workspace>/builtin_skills/google-workspace/scripts/google_api.py"
```
...
3. **Use the Gmail search syntax reference** for complex queries — load it with `skills show("google-workspace", file_path="references/gmail-search-syntax.md")`.

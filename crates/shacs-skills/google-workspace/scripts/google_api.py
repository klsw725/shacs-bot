#!/usr/bin/env python3
"""Google Workspace API CLI for shacs-bot.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing shacs-bot JSON contract and falls back to the Python client
...
# Ensure sibling modules (_shacs_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _shacs_home import get_shacs_home

SHACS_HOME = get_shacs_home()
TOKEN_PATH = SHACS_HOME / "google_token.json"
CLIENT_SECRET_PATH = SHACS_HOME / "google_client_secret.json"
...
def _gws_binary() -> str | None:
    override = os.getenv("SHACS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for shacs-bot")

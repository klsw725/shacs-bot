...
metadata.shacs.imported_from: productivity/linear
---
...
2. Set `LINEAR_API_KEY` in your environment (via shacs-bot config/env or your env config)
...
```bash
SCRIPT=$(dirname "$(find ~/.shacs-bot -path '*skills/productivity/linear/scripts/linear_api.py' 2>/dev/null | head -1)")/linear_api.py
...
- Always use `exec` tool with `curl` for API calls — do NOT use `web_fetch` or `browser`

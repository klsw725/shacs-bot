...
metadata.shacs.imported_from: creative/pixel-art
---
...
Call `ask_user` with 4 representative presets. Pick the set based on what the
...
`ask_user` and use the matching preset directly.
...
Do NOT call `ask_user` more than twice in a row. One for style, one for scene if
animation is on the table. If the user explicitly asked for a specific style
and scene in their message, skip `ask_user` entirely.
...
```python
import sys
sys.path.insert(0, "<workspace>/builtin_skills/pixel-art/scripts")
...
```bash
cd <workspace>/builtin_skills/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — install ffmpeg separately when needed)
...
- `ask_user` loop: call it at most twice per turn (style, then scene). Don't

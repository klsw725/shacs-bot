...
metadata.shacs.imported_from: creative/humanizer
---
...
2. **File** — user points at a file. Use `read_file` to load it, then `edit_file` or `write_file` to apply edits. For markdown docs in a repo, a targeted `edit_file` per section is cleaner than rewriting the whole file.
...
10. If the text came from a file, apply the edit with `edit_file` (targeted) or `write_file` (full rewrite) and show the user what changed.

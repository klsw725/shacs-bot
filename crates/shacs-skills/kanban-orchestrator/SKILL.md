...
metadata.shacs.imported_from: devops/kanban-orchestrator
disabled: true
...
- `configured kanban integration(assignee="<some-name>")` — sanity-check a single name. Returns an empty list (rather than an error) for an unknown assignee, so this only confirms a name you're already considering.
...
If *none* of those apply — it's a small one-shot reasoning task — use `spawn` instead or answer the user directly.
...
- **Never create dependent work as independent ready cards.** If a card must wait for another card, pass `parents=[...]` in the original `configured kanban integration` call. Do not create it first and link it later, and do not rely on prose like "wait for T1" inside the body.
...
```python
t1 = configured kanban integration(
...
t2 = configured kanban integration(
    title="research: Postgres performance vs current",
    assignee="<profile-A>",  # same profile, run in parallel
    body="Compare query latency, throughput, and scaling characteristics at our expected data volume (~500GB, 10k QPS peak). Sources: benchmark papers, public case studies, pgbench results if easy.",
)["task_id"]

t3 = configured kanban integration(
...
t4 = configured kanban integration(
...
If the task graph has dependencies, create the parent cards first, capture their returned ids, and include those ids in the child card's `parents` list during the child `configured kanban integration` call. Avoid creating all cards in parallel and linking them afterward; that creates a window where the dispatcher can claim a child before its inputs exist.
...
**Human-in-the-loop:** Any task can `configured kanban integration()` to wait for input. Dispatcher respawns after `/unblock`. The comment thread carries the full context.
...
**Argument order for links.** `configured kanban integration(parent_id=..., child_id=...)` — parent first. Mixing them up demotes the wrong task to `todo`.

**Don't pre-create the whole graph if the shape depends on intermediate findings.** If T3's structure depends on what T1 and T2 find, let T3 exist as a "synthesize findings" task whose own first step is to read parent handoffs and plan the rest. Orchestrators can spawn orchestrators.

**Tenant inheritance.** If `HERMES_TENANT` is set in your env, pass `tenant=os.environ.get("HERMES_TENANT")` on every `configured kanban integration` call so child tasks stay in the same namespace.

## Goal-mode cards (persistent workers)

By default a dispatched worker gets **one shot** at its card: it does its work, calls `configured kanban integration`/`configured kanban integration`, and exits. For open-ended cards where one turn rarely finishes the job, pass `goal_mode=True` to wrap that worker in a Ralph-style goal loop — the same engine behind the `/goal` slash command:
...
- Worker calls `configured kanban integration`/`configured kanban integration` itself → loop stops, normal lifecycle.
...
Hallucination warnings appear on tasks where a worker's `configured kanban integration(created_cards=[...])` claim included card ids that don't exist or weren't created by the worker's profile (the gate blocks the completion), or where the free-form summary references `t_<hex>` ids that don't resolve (advisory prose scan, non-blocking). Both produce audit events that persist even after recovery actions — the trail stays for debugging.

...
metadata.shacs.imported_from: software-development/node-inspect-debugger
---
...
- Node child processes or PTY bridge workers misbehave
...
```bash
cd the current shacs-bot checkout/ui-tui
...
### Debugging a running Node-based TUI

When a CLI spawns a Node-based TUI, attach to the Node process directly:

```bash
# 1. Launch the TUI command in another terminal
...
Those are Python, not Node — use the `python-debugpy` skill for them. Only Node portions (Ink UI, Node clients, tsx-run tests) use this skill.
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In shacs-bot, prefer the `tmux` skill for interactive stepping. Plain foreground `exec` works for one-shot commands, but it is not a PTY control surface.

...
`~/.shacs-bot/config.yaml` (defaults to `~/.shacs-bot/config.yaml` when `SHACS_HOME` is unset)
...
Restart the shacs-bot session for changes to take effect. The MCP connection is

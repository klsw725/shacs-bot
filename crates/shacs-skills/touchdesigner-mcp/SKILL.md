...
disabled: true
metadata.shacs.deferred_reason: Depends on Hermes-style MCP config files and setup scripts; keep as reference until shacs-bot MCP configuration support is documented for this workflow.
metadata.shacs.imported_from: creative/touchdesigner-mcp
---
...
```bash
bash "~/.shacs-bot/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to shacs-bot config (if missing)
...
3. **Restart shacs-bot session** to pick up the new MCP server

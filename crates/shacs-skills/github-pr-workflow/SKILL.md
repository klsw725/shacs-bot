...
metadata.shacs.imported_from: github/github-pr-workflow
---
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.shacs-bot/.env ] && grep -q "^GITHUB_TOKEN=" ~/.shacs-bot/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.shacs-bot/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
Use the agent's file tools (`write_file`, `edit_file`) to make changes, then commit:
...
After identifying the issue, use file tools (`edit_file`, `write_file`) to fix it:
...
3. Use `read_file` + `edit_file`/`write_file` → fix the code

...
metadata.shacs.imported_from: research/research-paper-writing
---
...
| **Low** (major unknowns) | Ask 1-2 targeted questions via `ask_user`, then draft |
...
**Load the `arxiv` skill** for structured paper discovery: `skills show("arxiv")`. It provides arXiv REST API search, Semantic Scholar citation graphs, author profiles, and BibTeX generation.
...
**For agent-based workflows**: Delegate each round's queries in parallel via `spawn`. Collect results, deduplicate, then generate the next round's queries from the combined learnings.
...
**Tools**: LaTeX (`beamerposter` package), slide deck tools, Figma, Canva.
...
## shacs-bot Integration

This skill is designed for the Hermes agent. It uses shacs-bot tools, delegation, scheduling, and memory for the full research lifecycle.

### Related Skills

Compose this skill with other shacs-bot skills for specific phases:

| Skill | When to Use | How to Load |
|-------|-------------|-------------|
| **arxiv** | Phase 1 (Literature Review): searching arXiv, generating BibTeX, finding related papers via Semantic Scholar | `skills show("arxiv")` |
| **subagent-driven-development** | Phase 5 (Drafting): parallel section writing with 2-stage review (spec compliance then quality) | `skills show("subagent-driven-development")` |
| **plan** | Phase 0 (Setup): creating structured plans before execution. Writes to `.shacs-bot/plans/` | `skills show("plan")` |
...
| **diagramming** | Phase 4-5: creating Excalidraw-based figures and architecture diagrams | `skills show("diagramming")` |
| **data-science** | Phase 4 (Analysis): Jupyter live kernel for interactive analysis and visualization | `skills show("data-science")` |
...
### shacs-bot Tools Reference

| Tool | Usage in This Pipeline |
|------|----------------------|
| **`exec`** | LaTeX compilation (`latexmk -pdf`), git operations, launching experiments (`nohup python run.py &`), process checks |
| **`process`** | Background experiment management: `process("start", ...)`, `process("poll", pid)`, `process("log", pid)`, `process("kill", pid)` |
| **`exec`** | Run Python for citation verification, statistical analysis, data aggregation. Has tool access via RPC. |
...
| **`spawn`** | **Parallel section drafting** — spawn isolated subagents for each section. Also for concurrent citation verification. |
...
| **`cron`** | Schedule experiment monitoring, deadline countdowns, automated arXiv checks. |
| **`ask_user`** | Ask the user targeted questions when blocked (venue choice, contribution framing). |
...
```
exec("ps aux | grep <pattern>")
→ exec("tail -30 <logfile>")
→ exec("ls results/")
→ exec("analyze results JSON, compute metrics")
→ exec("git add -A && git commit -m '<descriptive message>' && git push")
→ send_message("Experiment complete: <summary>")
```

**Parallel section drafting** (using delegation):
```
spawn("Draft the Methods section based on these experiment scripts and configs. 
...
spawn("Draft the Related Work section. Use web_search and web_fetch to 
  find papers. Verify every citation via Semantic Scholar. Group by methodology.")

spawn("Draft the Experiments section. Read all result files in results/. 
...
**Citation verification** (using exec):
```python
# In exec:
...
3. exec("git log --oneline -10")      # Check recent commits
4. exec("ps aux | grep python")       # Check running experiments
5. exec("ls results/ | tail -20")     # Check for new results
6. Report status to user, ask for direction
```

### Cron Monitoring with `cron`

Use the `cron` tool to schedule periodic experiment checks:
...
Use `ask_user` for targeted questions when genuinely blocked:

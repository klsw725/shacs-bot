...
metadata.shacs.imported_from: autonomous-ai-agents/hermes-agent
disabled: true
...
/reload-skills       Re-scan ~/.shacs-bot/skills/ for added/removed skills
...
```
~/.shacs-bot/config.yaml       Main configuration
~/.shacs-bot/.env              API keys and secrets
~/.shacs-bot/skills/        Installed skills
~/.shacs-bot/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.shacs-bot/state.db          Canonical session store (SQLite + FTS5)
~/.shacs-bot/logs/             Gateway and error logs
~/.shacs-bot/auth.json         OAuth tokens and credential pools
~/.shacs-bot/hermes-agent/     Source code (if git-installed)
```

Profiles use `~/.shacs-bot/profiles/<name>/` with the same layout.
...
| `exec` | `backend` (local/docker/ssh/modal), `cwd`, `timeout` (180) |
...
| `exec` | Shell commands and process management |
...
| `ask_user` | Ask user clarifying questions |
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.shacs-bot/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
### When to Use This vs spawn

| | `spawn` | Spawning `hermes` process |
|-|-----------------|--------------------------|
...
```
exec(command="hermes chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# Background for long tasks:
exec(command="hermes chat -q 'Set up CI/CD for ~/myapp'", background=true)
```
...
```
# Start
exec(command="tmux new-session -d -s agent1 -x 120 -y 40 'hermes'", timeout=10)

# Wait for startup, then send a message
exec(command="sleep 8 && tmux send-keys -t agent1 'Build a FastAPI auth service' Enter", timeout=15)

# Read output
exec(command="sleep 20 && tmux capture-pane -t agent1 -p", timeout=5)

# Send follow-up
exec(command="tmux send-keys -t agent1 'Add rate limiting middleware' Enter", timeout=5)

# Exit
exec(command="tmux send-keys -t agent1 '/exit' Enter && sleep 2 && tmux kill-session -t agent1", timeout=10)
```
...
```
# Agent A: backend
exec(command="tmux new-session -d -s backend -x 120 -y 40 'hermes -w'", timeout=10)
exec(command="sleep 8 && tmux send-keys -t backend 'Build REST API for user management' Enter", timeout=15)

# Agent B: frontend
exec(command="tmux new-session -d -s frontend -x 120 -y 40 'hermes -w'", timeout=10)
exec(command="sleep 8 && tmux send-keys -t frontend 'Build React dashboard for user management' Enter", timeout=15)

# Check progress, relay context between them
exec(command="tmux capture-pane -t backend -p | tail -30", timeout=5)
exec(command="tmux send-keys -t frontend 'Here is the API schema from the backend agent: ...' Enter", timeout=5)
```
...
exec(command="tmux new-session -d -s resumed 'hermes --continue'", timeout=10)

# Resume specific session
exec(command="tmux new-session -d -s resumed 'hermes --resume 20260225_143052_a1b2c3'", timeout=10)
```

### Tips

- **Prefer `spawn` for quick subtasks** — less overhead than spawning a full process
- **Use `-w` (worktree mode)** when spawning agents that edit code — prevents git conflicts
- **Set timeouts** for one-shot mode — complex tasks can take 5-10 minutes
- **Use `hermes chat -q` for fire-and-forget** — no PTY needed
- **Use tmux for interactive sessions** — raw PTY mode has `\r` vs `\n` issues with prompt_toolkit
- **For scheduled tasks**, use the `cron` tool instead of spawning — handles delivery and retry
...
- **Single:** `spawn(goal, context, toolsets)`.
- **Batch:** `spawn(tasks=[{goal, ...}, ...])` runs children in
  parallel, capped by `delegation.max_concurrent_children` (default 3).
- **Roles:** `leaf` (default; cannot re-delegate) vs `orchestrator`
  (can spawn its own workers, bounded by `delegation.max_spawn_depth`).
- **Not durable.** If the parent is interrupted, the child is
  cancelled. For work that must outlive the turn, use `cron` or
  `exec(background=True, notify_on_complete=True)`.
...
the `cron` tool, the `hermes cron` CLI (`list`, `add`, `edit`,
...
- **Telemetry:** sidecar at `~/.shacs-bot/skills/.usage.json` holds
...
see a focused configured kanban integration toolset gated by `HERMES_KANBAN_TASK`, and
orchestrator profiles can opt into the broader `kanban` toolset. Normal
sessions still have zero configured kanban integration schema footprint unless configured.

- **CLI verbs (common):** `init`, `create`, `list` (alias `ls`),
  `show`, `assign`, `link`, `unlink`, `comment`, `complete`, `block`,
  `unblock`, `archive`, `tail`. Less common: `watch`, `stats`, `runs`,
  `log`, `dispatch`, `daemon`, `gc`.
- **Worker/orchestrator toolset:** `configured kanban integration`, `configured kanban integration`,
  `configured kanban integration`, `configured kanban integration`, `configured kanban integration`, `configured kanban integration`,
  `configured kanban integration`; profiles that explicitly enable the `kanban` toolset
  outside a dispatcher-spawned task also get `configured kanban integration` and
  `configured kanban integration` for board routing.
...
every shacs-bot tool and most Windows APIs. Prefer forward slashes in code
...
Check logs first:
```bash
grep -i "failed to send\|error" ~/.shacs-bot/logs/gateway.log | tail -20
```
...
| Gateway logs | `~/.shacs-bot/logs/gateway.log` |
| Session files | `hermes sessions browse` (reads state.db) |
| Source code | `~/.shacs-bot/hermes-agent/` |
...
Config: `~/.shacs-bot/config.yaml` (settings), `~/.shacs-bot/.env` (API keys).
...
All handlers must return JSON strings. Use `get_hermes_home()` for paths, never hardcode `~/.shacs-bot`.
...
- Tests auto-redirect `SHACS_HOME` to temp dirs — never touch real `~/.shacs-bot/`
...
- **Local terminal backend** → emit host info (OS, `$HOME`, cwd) + Windows-specific notes (hostname ≠ username, `exec` uses bash not PowerShell).
...
- **Key fact for prompt authoring:** when `TERMINAL_ENV != "local"`, *every* file tool (`read_file`, `write_file`, `edit_file`, `grep` / `glob`) runs inside the backend container, not on the host. The system prompt must never describe the host in that case — the agent can't touch it.

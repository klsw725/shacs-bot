...
metadata.shacs.imported_from: autonomous-ai-agents/kanban-codex-lane
disabled: true
...
This skill defines the lightweight Hermes+Codex dual-lane convention for Kanban workers. Hermes is always the task owner: it calls `configured kanban integration`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `configured kanban integration` or `configured kanban integration` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.
...
1. Hermes owns the Kanban lifecycle. Codex must never call `configured kanban integration`, `configured kanban integration`, `configured kanban integration`, gateway messaging, or any Hermes board CLI as a substitute for the worker.
...
Send a Kanban heartbeat every few minutes for lanes longer than two minutes, e.g. `configured kanban integration(note="Codex lane running in <WORKTREE>; waiting for tests/diff")`.
...
## configured kanban integration Metadata Schema
...
- [ ] `configured kanban integration.metadata.codex_lane` follows the schema above.

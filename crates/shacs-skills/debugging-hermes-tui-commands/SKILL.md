...
metadata.shacs.imported_from: software-development/debugging-hermes-tui-commands
disabled: true
...
1. **Check if the command exists in the TUI frontend:**
   ```bash
   grep / glob --pattern "/commandname" --file_glob "*.ts" --path ui-tui/
   grep / glob --pattern "/commandname" --file_glob "*.tsx" --path ui-tui/
   ```
...
2. **Examine the TUI command definition:**
   ...
   # If not there:
   grep / glob --pattern "commandname" --path ui-tui/src/app/slash/commands --target files
   ```

3. **Check if the command exists in the Python backend:**
   ```bash
   grep / glob --pattern "CommandDef" --file_glob "*.py" --path hermes_cli/
   grep / glob --pattern "commandname" --path hermes_cli/commands.py --context 3
   ```

4. **Examine the gateway implementation:**
   ```bash
   grep / glob --pattern "complete.slash|slash.exec" --path tui_gateway/
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.shacs-bot/config.yaml`)

...
metadata.shacs.imported_from: software-development/python-debugpy
---
...
- A long-running gateway, daemon, or child process misbehaves and you cannot restart it
...
For long-lived processes: a gateway, daemon, or already-misbehaving process that cannot be restarted cleanly.

### Setup

```bash
source the current shacs-bot checkout/.venv/bin/activate
pip install debugpy
```
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "the current shacs-bot checkout" }
  ]
}
```
...
### Gateway subprocess spawned by a TUI
If a gateway runs as a child of a Node TUI, use one of these options:

**A. Source-edit the gateway:**
```python
# gateway/server.py near the top of serve()
import debugpy
debugpy.listen(("127.0.0.1", 5678))
debugpy.wait_for_client()
```
Start the TUI. It will appear frozen while its backend waits. Attach a client; execution resumes when you `continue`.

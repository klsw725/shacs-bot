...
metadata.shacs.imported_from: software-development/systematic-debugging
---
...
**Action:** Use `read_file` on the relevant source files. Use `grep` / `glob` to find the error string in the codebase.
...
**Action:** Use the `exec` tool to run the failing test or trigger the bug:
...
grep / glob("function_name(", path="src/", file_glob="*.py")

# Find where the variable is set
grep / glob("variable_name\\s*=", path="src/", file_glob="*.py")
```
...
```python
grep / glob("similar_pattern", path="src/", file_glob="*.py")
```
...
Use these shacs-bot tools during Phase 1:

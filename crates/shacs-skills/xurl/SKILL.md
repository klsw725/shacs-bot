...
metadata.shacs.imported_from: social-media/xurl
---
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by shacs-bot tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Hermes Docker layout, `/opt/data` is `SHACS_HOME`, but shacs-bot tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Hermes-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, shacs-bot tool calls will not see the credentials.
...
- Hermes adaptation: reformatted for shacs-bot skill conventions; safety guardrails preserved verbatim.

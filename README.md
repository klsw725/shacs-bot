...
- **채널별 응답 렌더링** — 같은 assistant 응답도 Slack/Discord/Telegram/Email 규칙에 맞게 자연스럽게 표현
...
| 채널 | 필요한 것 | 비고 |
|------|----------|------|
| **Telegram** | Bot token ([@BotFather](https://t.me/BotFather)) | 추천, 가장 안정적 / HTML-safe 렌더링 |
| **Discord** | Bot token + Message Content intent | 스레드 기반 응답 지원 / 긴 응답 분할 |
| **WhatsApp** | QR 코드 스캔 (Node.js ≥18 필요) | TypeScript 브리지 사용 |
| **Slack** | Bot token + App-Level token (Socket Mode) | mrkdwn 렌더링 / 긴 응답 분할 |
| **Feishu (飞书)** | App ID + App Secret | WebSocket, 공인 IP 불필요 |
| **DingTalk (钉钉)** | App Key + App Secret | Stream Mode |
| **QQ** | App ID + App Secret | 개인 메시지만 지원 |
| **Email** | IMAP/SMTP 자격증명 | 이메일 어시스턴트 / subject-body 분리 |

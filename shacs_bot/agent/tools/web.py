...
def _to_markdown(html: str) -> str:
    ...
    text = re.sub(pattern=r"</(p|div|section|article)>", repl="\n\n", string=text, flags=re.I)
    text = re.sub(pattern=r"<(br|hr)\s*/?>", repl="\n", string=text, flags=re.I)
...
class WebSearchTool(Tool):
    ...
    async def _search_duckduckgo(self, query: str, n: int) -> str:
        """BRAVE_API_KEY 미설정 시 DuckDuckGo로 폴백 검색."""
        try:
            from ddgs import DDGS

            ddgs = DDGS(timeout=10)
            raw: list[dict[str, str]] = await asyncio.to_thread(ddgs.text, query, max_results=n)
            if not raw:
                return f"'{query}'에 대한 검색 결과가 없습니다."
...
class WebFetchTool(Tool):
    ...
    parameters = {
        "type": "object",
        "properties": {
            "url": {"type": "string", "description": "가져올 웹 페이지의 URL"},
            "extractMode": {
                "type": "string",
                "enum": ["markdown", "text"],
                "default": "markdown",
    ...
    async def execute(
        ...
        if not is_valid:
            return json.dumps({"error": f"URL 검증 실패: {error_msg}", "url": url})

        try:
            logger.debug("WebFetch: {}", "proxy enabled" if self._proxy else "direct connection")
            async with httpx.AsyncClient(
                follow_redirects=True, max_redirects=MAX_REDIRECTS, timeout=30.0, proxy=self._proxy
            ) as client:

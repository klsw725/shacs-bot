    ...
    for idx, msg in enumerate(messages):
        ...
        if role == "system":
            ...
            for tool_call in msg.get("tool_calls", []) or []:
                fn: dict[str, Any] = tool_call.get("function") or {}
...
def _prompt_cache_key(messages: list[dict[str, Any]]) -> str:
    raw: str = json.dumps(messages, ensure_ascii=True, sort_keys=True)
    ...
    async for event in _iter_sse(response):
        ...
        if event_type == "response.output_item.added":
            ...
            if item.get("type") == "function_call":
                ...
                buf: dict[str, Any] = tool_call_buffers.get(call_id) or {}
                args_raw: str = buf.get("arguments") or item.get("arguments") or "{}"
                try:
            ...
            resp_data: dict[str, Any] = event.get("response") or {}
            status: str | None = resp_data.get("status")
            finish_reason = _map_finish_reason(status)
            raw_usage: dict[str, Any] = resp_data.get("usage") or {}
            if raw_usage:
                usage = {
